...
def Try_MakeQRCode(QRCodeLink:str="none"):
    try:
        #import segno -> create a QR code -> save it to a temp file -> read -> delete file -> return SVG text
        import segno
        TempSVGpath = os.path.join(os.getcwd(),'VIPERLocalQRCode_TEMP.svg')
        qrcode = segno.make(QRCodeLink)
        qrcode.save(TempSVGpath, border=0, scale=10)  # No border
        qrcodefile = open(TempSVGpath, "r")
        QRFinal = qrcodefile.read()
        qrcodefile.close()
...
def VIPER_SDump_For_Client(RawCallbackURL):
    ...
    sdumpQRCode = Try_MakeQRCode("SDUMP")
    if sdumpQRCode:
        with open("SDumps_QRCode.svg","w") as SDumpFile:
            SDumpFile.write(sdumpQRCode)
