...
import Basic_Admin_Panel_Module_Script #import user script
...
def LIMSReferralName():
    return Basic_Admin_Panel_Module_Script.ModuleName().strip().replace(" ", "_")
...
def ModuleHomepage():
    ...
    #else:
    return render_template("Basic_Admin_Panel_Homepage.html", LIMSRef=LIMSReferralName(), LIMSPreRoute=LIMSPreRoute, LocalIPAddress=VIPER_Utility.VIPER_DecodeLoginCookie(request.cookies.get('VIPER_LocalUserInfo'))["LocalIPAddress"] )
...
def InduceButtonPageExample():
    ...
    """
    return Basic_Admin_Panel_Module_Script.Process_User_Request(request)
