...
import Method_Checker_Module_Script #import user script
...
def LIMSReferralName():
    return Method_Checker_Module_Script.ModuleName().strip().replace(" ", "_")
...
def ModuleHomepage():
    ...
    #else:
    return render_template("Method_Checker_Homepage.html", LIMSRef=LIMSReferralName(), LIMSPreRoute=LIMSPreRoute, LocalIPAddress=VIPER_Utility.VIPER_DecodeLoginCookie(request.cookies.get('VIPER_LocalUserInfo'))["LocalIPAddress"] )
...
def InduceButtonPageExample():
    ...
    """
    return Method_Checker_Module_Script.Process_User_Request(request)
