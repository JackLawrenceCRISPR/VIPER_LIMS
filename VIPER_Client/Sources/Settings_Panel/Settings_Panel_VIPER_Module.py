...
##Module pages 
Module_Page = Blueprint(LIMSReferralRoute()+LIMSReferralName(), __name__, template_folder=os.path.dirname(os.path.realpath(__file__)), static_folder=os.path.join(os.path.dirname(os.path.realpath(__file__)), "Static"), static_url_path="/"+LIMSPreRoute+LIMSReferralName()+"/Static")

@Module_Page.route("/"+LIMSReferralRoute()+LIMSReferralName(), methods = ["POST", "GET"])
def ModuleHomepage():
    ...
    #else:
    return render_template("Settings_Panel_Homepage.html", LIMSRef=LIMSReferralName(), LIMSPreRoute=LIMSReferralRoute(), LocalIPAddress=VIPER_Utility.VIPER_DecodeLoginCookie(request.cookies.get('VIPER_LocalUserInfo'))["LocalIPAddress"] )
       
       
@Module_Page.route("/"+LIMSReferralRoute()+LIMSReferralName()+"/InduceButton", methods=['POST'])
def InduceButtonPageExample():
    ...
    """
    return Settings_Panel_Module_Script.Process_User_Request(request,LIMSReferralRoute(),LIMSReferralName())
