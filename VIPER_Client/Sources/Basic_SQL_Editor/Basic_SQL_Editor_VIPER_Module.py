...
import Basic_SQL_Editor_Module_Script #import user script

def LIMSReferralName():
    return Basic_SQL_Editor_Module_Script.ModuleName().strip().replace(" ", "_")

##Module pages 
Module_Page = Blueprint(LIMSReferralName(), __name__, template_folder=os.getcwd(), static_folder=os.path.join(os.path.dirname(os.path.realpath(__file__)), "Static"), static_url_path="/"+LIMSReferralName()+"/Static")
...
def ModuleHomepage():
    ...
    #else:
    return render_template("Basic_SQL_Editor_Homepage.html", LIMSRef=LIMSReferralName(), LocalIPAddress=VIPER_Utility.VIPER_DecodeLoginCookie(request.cookies.get('VIPER_LocalUserInfo'))["LocalIPAddress"] )
...
def InduceButtonPageExample():
    ...
    """
    return Basic_SQL_Editor_Module_Script.Process_User_Request(request)
