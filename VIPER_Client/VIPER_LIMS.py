...
def Install_Module(ModuleName : str):
    ...
    """
    print("Requests module not found, would you like to install? [Y/N]")
...
try:
    os.chdir(os.path.join(VIPER_Directory, "Logs", Filename_Today()))
except:
    os.mkdir(os.path.join(VIPER_Directory, "Logs", Filename_Today()))
    os.chdir(os.path.join(VIPER_Directory, "Logs", Filename_Today()))
...
sys.path.append(os.path.join(VIPER_Directory, "Sources", "VIPER"))
...
def ScanForVIPERModules(LML:list, ScanDir:str=False): 
    if ScanDir:
        os.chdir(ScanDir)
    ScanDir = os.getcwd()
    for MF in [MF for MF in list(filter(os.path.isdir, os.listdir(os.getcwd()))) if len( glob.glob( os.path.join(MF, "*_VIPER_Module.py") ) )>0]: # :^)
        ...
        if os.path.exists(MFFile):
            if not os.path.exists(os.path.join(FMF,"INACTIVE")):
                ...
                os.chdir(FMF)
                sys.path.append(FMF)
                LML.append( [__import__(MFFile[len(FMF)+1:-3]), not os.path.exists(os.path.join(FMF,"UNLISTED")) ] ) 
...
def Exit_LIMS():
    ...
    return r"<H1>Restarting LIMS</h1>" 
...
for ModuleName in LML:
    if ModuleName[1]:
        ModuleLink = f"http://{LocalIPAddress}:7800/"+ModuleName[0].LIMSReferralName() #A module's location on the local Flask instance
